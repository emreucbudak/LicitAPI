...
using Licit.WalletService.Application.Validators.Wallet.Commands.Freeze;

namespace Licit.WalletService.UnitTests.Application.Validators;

public class FreezeFundsCommandValidatorTests
{
    private readonly FreezeFundsCommandValidator _validator = new();
    ...
    {
        var result = await _validator.TestValidateAsync(new FreezeFundsCommandRequest(Guid.NewGuid(), 0, Guid.NewGuid(), null));
