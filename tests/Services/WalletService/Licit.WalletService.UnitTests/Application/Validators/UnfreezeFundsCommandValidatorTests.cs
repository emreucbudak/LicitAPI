...
using Licit.WalletService.Application.Validators.Wallet.Commands.Unfreeze;
...
public class UnfreezeFundsCommandValidatorTests
{
    ...
    {
        var result = await _validator.TestValidateAsync(new UnfreezeFundsCommandRequest(Guid.NewGuid(), 100, Guid.NewGuid(), null));
    ...
    {
        var result = await _validator.TestValidateAsync(new UnfreezeFundsCommandRequest(Guid.Empty, 100, Guid.NewGuid(), null));
    ...
    {
        var result = await _validator.TestValidateAsync(new UnfreezeFundsCommandRequest(Guid.NewGuid(), 100, Guid.Empty, null));
