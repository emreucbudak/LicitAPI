...
public static class AuthLoginTwoFactorEmailTemplate
{
    ...
    public static string BuildBody(AuthLoginTwoFactorEmailEvent emailEvent)
    {
        ...
        var greetingText = string.IsNullOrWhiteSpace(emailEvent.UserName)
            ? "Licit hesabınıza girişinizi tamamlamak için aşağıdaki tek kullanımlık kodu kullanın."
            : $"Merhaba {WebUtility.HtmlEncode(emailEvent.UserName)}, Licit hesabınıza girişinizi tamamlamak için aşağıdaki tek kullanımlık kodu kullanın.";
        var expiryText = emailEvent.ExpiresAt.HasValue
            ? $"Kod {emailEvent.ExpiresAt.Value:dd.MM.yyyy HH:mm} tarihine kadar geçerlidir."
            : "Kod kısa süre için geçerlidir.";
        ...
        body.AppendLine("  <title>Licit giriş doğrulama kodu</title>");
        body.AppendLine("</head>");
        body.AppendLine("<body style=\"margin:0;padding:0;background:#0b1326;color:#f8fafc;font-family:Arial,Helvetica,sans-serif;\">");
        body.AppendLine("  <div style=\"display:none;max-height:0;overflow:hidden;opacity:0;color:transparent;\">Licit giriş kodunuz hazır.</div>");
        ...
        body.AppendLine("                  <td align=\"right\" style=\"font-size:12px;font-weight:800;letter-spacing:0.16em;text-transform:uppercase;color:#4edea3;\">Güvenli Giriş</td>");
        body.AppendLine("                </tr>");
        ...
        body.AppendLine("                    <div style=\"display:inline-block;padding:8px 12px;border-radius:999px;background:#1f2a44;color:#c0c1ff;font-size:12px;font-weight:800;letter-spacing:0.14em;text-transform:uppercase;\">Tek kullanımlık kod</div>");
        body.AppendLine("                    <h1 style=\"margin:20px 0 12px 0;color:#ffffff;font-size:30px;line-height:1.18;font-weight:900;letter-spacing:-0.04em;\">Giriş kodunuz hazır</h1>");
        ...
        body.AppendLine("                      <tr>");
        body.AppendLine("                        <td align=\"center\" style=\"padding:24px 18px 8px 18px;color:#94a3b8;font-size:12px;font-weight:800;letter-spacing:0.16em;text-transform:uppercase;\">Doğrulama kodu</td>");
        body.AppendLine("                      </tr>");
        body.AppendLine("                      <tr>");
        ...
        body.AppendLine($"                          <strong style=\"color:#ffffff;\">Süre:</strong> {expiryText}");
        body.AppendLine("                        </td>");
        body.AppendLine("                      </tr>");
        body.AppendLine("                    </table>");
        body.AppendLine("                  </td>");
        ...
        body.AppendLine($"                    <p style=\"margin:0 0 10px 0;color:#94a3b8;font-size:14px;line-height:1.65;\">Bu kod <strong style=\"color:#f8fafc;\">{safeEmail}</strong> adresi için oluşturuldu.</p>");
        body.AppendLine("                    <p style=\"margin:0;color:#94a3b8;font-size:13px;line-height:1.65;\">Bu girişi siz başlatmadıysanız e-postayı yok sayabilir, hesabınızı güvende tutmak için şifrenizi değiştirebilirsiniz.</p>");
        body.AppendLine("                  </td>");
        body.AppendLine("                </tr>");
        body.AppendLine("              </table>");
        body.AppendLine("            </td>");
        body.AppendLine("          </tr>");
        body.AppendLine("          <tr>");
        body.AppendLine("            <td align=\"center\" style=\"padding:18px 10px 0 10px;color:#64748b;font-size:12px;line-height:1.5;\">Licit'te güvenli teklif deneyimi için bu kodu kimseyle paylaşmayın.</td>");
        body.AppendLine("          </tr>");
        body.AppendLine("        </table>");
        body.AppendLine("      </td>");
        body.AppendLine("    </tr>");
